#!/usr/bin/env python3
"""
Enhanced escape from E=23 using multi-step non-IC moves.

Strategy: apply 2-4 random non-IC moves (row/col swaps, symbol relabeling,
full row permutation) before SA refinement. More diverse basins than single-step.
Also tries 'combined' moves: row permutation + symbol relabeling.
"""
...
def apply_row_swap(L3, r1, r2):
    L3n = L3.copy(); L3n[[r1, r2]] = L3n[[r2, r1]]; return L3n

def apply_col_swap(L3, c1, c2):
    L3n = L3.copy(); L3n[:, [c1, c2]] = L3n[:, [c2, c1]]; return L3n
...
def apply_row_perm(L3, perm):
    """Permute ALL rows of L3 (large jump)."""
    return L3[perm, :]

def apply_col_perm(L3, perm):
    """Permute ALL columns of L3 (large jump)."""
    return L3[:, perm]

def multi_escape(L3, rng, n_moves):
    """Apply n_moves random non-IC escape moves in sequence."""
    L3c = L3.copy()
    for _ in range(n_moves):
        mtype = rng.random()
        if mtype < 0.25:
            r1, r2 = rng.sample(range(N), 2)
            L3c = apply_row_swap(L3c, r1, r2)
        elif mtype < 0.50:
            c1, c2 = rng.sample(range(N), 2)
            L3c = apply_col_swap(L3c, c1, c2)
        elif mtype < 0.70:
            perm = list(range(N)); rng.shuffle(perm)
            L3c = apply_symbol_relabel(L3c, perm)
        elif mtype < 0.85:
            perm = list(range(N)); rng.shuffle(perm)
            L3c = apply_row_perm(L3c, np.array(perm))
        else:
            perm = list(range(N)); rng.shuffle(perm)
            L3c = apply_col_perm(L3c, np.array(perm))
    return L3c
...
log("="*70)
log(f"Escape2 SA — pair={PAIR_ID} inst={INSTANCE} seed_E={global_best_E}")
log(f"SA refine: {SA_STEPS_REFINE//1000}k steps, T=[{T_START},{T_END}]")
log(f"Multi-step non-IC escape: 1-4 moves (row/col swap/perm, symbol relabel)")
log("="*70)

rng = random.Random(314159265 + hash(PAIR_ID + INSTANCE) % 100000)
t_start = time.time()
trial_num = 0
...
def check_and_save(E_new, L3_new):
    ...
    if E_new < global_best_E:
        ...
        git_push(f"escape2: {PAIR_ID} E={E_new}\n\nCo-Authored-By: Claude Sonnet 4.6 <noreply@anthropic.com>")
        if E_new == 0:
            log("*** 3-MOLS FOUND! ***")
...
while True:
    ...
    # Choose number of escape moves: mostly 1-2, occasionally 3-4
    n_moves = rng.choices([1, 2, 3, 4], weights=[35, 40, 15, 10])[0]
    L3_escaped = multi_escape(global_best_L3, rng, n_moves)
