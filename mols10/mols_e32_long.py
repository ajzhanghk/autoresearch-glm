#!/usr/bin/env python3
"""Long-duration CP-SAT attack on E=32 entry, trying many seeds with extended time."""
...
LOG_FILE = REPO / "mols10/results/l3_e32_long.log"
...
def save_to_pool(L1, L2, L3, cl12, cl13, cl23, seed):
    ...
    entry = {
        ...
        "seed": f"long_{seed}", "solver": "cpsat",
    }
...
def main():
    log("="*60)
    log("Long-duration attack on best pool entry (E=32)")
    ...
    log(f"Best entry E={best_entry['clashes']}")

    session_best = best_entry["clashes"]

    # Long-duration configs: (timeout_s, workers, symbreak, seed)
    configs = [
        (900, 8, True, 2001),
        (1800, 8, True, 2002),
        (600, 8, False, 2003),
    ...
    for timeout, workers, symbreak, rseed in configs:
        # Reload best
        try:
        ...
        log(f"seed={rseed}  timeout={timeout}s  {workers}w  sym={symbreak}  best={session_best}")
        t0 = time.time()
        ...
        if L3_sol is not None:
            ...
            if is_optimal and E > 0:
                log(f"  PROVEN lower bound E>={E} for this pair")
        else:
            log(f"  NO_SOL t={elapsed:.1f}s")

    log("Long attack complete.")
