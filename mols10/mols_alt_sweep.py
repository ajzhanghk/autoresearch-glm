#!/usr/bin/env python3
"""Attack an alternative pool entry (by index) with 900s CP-SAT sweeps."""
...
parser.add_argument("--pool-index", type=int, default=1)
...
LOG_FILE = REPO / f"mols10/results/l3_alt_sweep_p{args.pool_index}{args.log_suffix}.log"
...
def save_to_pool(L1, L2, L3, cl12, cl13, cl23, seed):
    ...
    entry = {
        ...
        "seed": f"alt_{args.pool_index}_{seed}", "solver": "cpsat",
    }
...
def main():
    log("="*60)
    log(f"Alt-pair sweep: pool[{args.pool_index}] s={args.seed_start}-{args.seed_end} t={args.timeout}s {args.workers}w")

    pool = json.loads(TRIPLE_MISS_FILE.read_text()) if TRIPLE_MISS_FILE.exists() else []
    if args.pool_index >= len(pool):
        log(f"ERROR: pool_index={args.pool_index} out of range (pool size={len(pool)})")
        return

    entry = pool[args.pool_index]
    L1 = np.array(entry["L1"], dtype=np.int8).reshape(N, N)
    L2 = np.array(entry["L2"], dtype=np.int8).reshape(N, N)
    hint = np.array(entry["L3"], dtype=np.int8).reshape(N, N)
    log(f"Target: pool[{args.pool_index}] E={entry['clashes']} cl12={entry.get('cl12','?')}")

    # Track global best separately
    global_best = min(pool, key=lambda e: e["clashes"])["clashes"]
    session_best = entry["clashes"]
    ...
    for rseed in range(args.seed_start, args.seed_end):
        ...
        try:
            ...
            gb = min(pool_data, key=lambda e: e["clashes"])["clashes"]
            if gb < global_best:
                global_best = gb
                log(f"  → global best improved to E={global_best}")
        except Exception:
            pass

        log(f"  seed={rseed}  session_best={session_best}  global_best={global_best}")
        t0 = time.time()
        result = build_and_solve(L1, L2, N, hint_L3=hint,
            timeout_s=args.timeout, num_workers=args.workers,
            symbreak=True, random_seed=rseed)
        ...
        if L3_sol is not None:
            ...
            if E < session_best:
                ...
                log(f"  ★★★ NEW BEST for this pair: E={E}  saved={saved} ★★★")
            elif E <= session_best + 3:
                hint = L3_sol.copy()

    log(f"Alt-pair sweep done: pair_best={session_best}")
