#!/usr/bin/env python3
"""Long-duration attack on E=31 pair targeting E<=30."""
...
LOG_FILE = REPO / f"mols10/results/l3_e31_long_{args.seed}.log"
...
def save_to_pool(L1, L2, L3, cl12, cl13, cl23, tag):
    ...
    entry = {
        ...
        "seed": f"long31_{tag}", "solver": "cpsat",
    }
...
def main():
    log("="*60)
    log(f"Long-duration attack on E=31 pair (seed={args.seed})")
    ...
    for timeout, workers, symbreak, rseed in configs:
        ...
        if L3_sol is not None:
            ...
            if is_optimal and E > 0:
                log(f"  PROVED: min E>={E} for this (L1,L2) pair")
        else:
