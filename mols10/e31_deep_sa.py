#!/usr/bin/env python3
"""
Deep SA on all E=31 non-mdecomp pairs, seeking E<26.
Pairs with confirmed E=31: tv_222, tv_223, tv_254, tv_270, iso2_tv21_0_1
Also includes iso_tv_21_0 (E=26) seeking E<26.

Strategy: Start from best known L3 for each pair with minimal perturbation,
run long SA. Also try cross-seeds.
"""
...
LOG       = REPO / "mols10/results/e31_deep_sa.log"
BEST_FILE = REPO / "mols10/results/e31_deep_sa_best.json"
BRANCH    = "claude/mols-order-10-search-yfQXK"

T_START = 10.0
T_END   = 0.03
N_STEPS = 10_000_000  # 10M steps per run

# Highest priority: pairs with E=31 or better
PAIR_IDS = [
    'tv_222',          # survey E=31
    'tv_223',          # survey E=31
    'tv_254',          # survey E=31
...
for fname, label in [
    ...
    if fpath.exists():
        ...
        seeds.append((np.array(d['L3'], dtype=np.int8).reshape(N, N), label, d['E']))

# Load tv_unique_survey seeds for E=31 pairs
...
# Load e31_deep_sa_best seeds
e31_seeds = {}
if BEST_FILE.exists():
    best_data = json.loads(BEST_FILE.read_text())
    for pid, entry in best_data.items():
        if entry.get('L3') and entry['E'] <= 34:
            e31_seeds[pid] = (np.array(entry['L3'], dtype=np.int8).reshape(N, N), entry['E'])

log("=" * 70)
log(f"Deep SA on E=31 pairs (goal: E<26)")
log(f"N_STEPS={N_STEPS//1_000_000}M, T: {T_START}→{T_END}")
log(f"Generic seeds: {[(s[1], s[2]) for s in seeds]}")
log(f"Survey seeds: {list(tv_seeds.keys())}")
log("=" * 70)
...
while True:
    # Reload survey seeds to catch updates
    if tv_survey_file.exists():
        tv_data_new = json.loads(tv_survey_file.read_text())
        for pid, entry in tv_data_new.items():
    ...
    for pair_id in PAIR_IDS:
        ...
        if BEST_FILE.exists():
            ...
            e31_seeds = {}
            for pid2, entry in best_per_pair.items():
                if entry.get('L3') and entry['E'] <= 34:
                    e31_seeds[pid2] = (np.array(entry['L3'], dtype=np.int8).reshape(N, N), entry['E'])
        ...
        # Build start options: best pair-specific first, then cross-pair
        pair_seeds = []
        if pair_id in tv_seeds:
            pair_seeds.append((tv_seeds[pair_id][0], 'survey', tv_seeds[pair_id][1]))
        if L3_pair_best is not None:
            pair_seeds.append((L3_pair_best, 'e31best', pair_best))
        pair_seeds.sort(key=lambda x: x[2])
        # Add cross-pair E=31 seeds
        cross31 = [(L3, f'{pid}_E31', E) for pid, (L3, E) in tv_seeds.items() if E <= 32 and pid != pair_id]
        cross31.sort(key=lambda x: x[2])
        all_starts = pair_seeds + [(s[0], s[1], s[2]) for s in seeds] + cross31[:3]

        L3_seed, seed_label, seed_E = all_starts[pair_run % len(all_starts)]

        L3 = L3_seed.copy()
        # Use low perturbation for good seeds, higher for cross-seeds
        max_p = 3 if seed_E <= 32 else 6
            ...
            if dE < 0 or random.random() < math.exp(-dE / T):
                ...
                if E_curr < E_best_run:
                    ...
                    if E_best_run < pair_best:
                        ...
                        git_push(f"e31_deep_sa: {pair_id} E={pair_best}\n\nCo-Authored-By: Claude Sonnet 4.6 <noreply@anthropic.com>")
                        if pair_best == 0:
                            log("*** 3-MOLS FOUND! ***"); sys.exit(0)

        elapsed = time.time() - t0
        log(f"run={run_global:4d} {pair_id}: perturb={n_perturb} seed={seed_label}({seed_E}) "
            f"best={E_best_run} pair_best={pair_best} acc={accepted/max(tried,1)*100:.0f}% "
