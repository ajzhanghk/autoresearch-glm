#!/usr/bin/env python3
"""
Sweep many solver seeds on the best pool entry (E=32) to find E<=31.
Runs 40 solver seeds at 300s each with 4 workers. Saves any improvement.
"""
...
parser.add_argument("--seed-end", type=int, default=40)
...
LOG_FILE = REPO / f"mols10/results/l3_e32_sweep{args.log_suffix}.log"
...
def save_to_pool(L1, L2, L3, cl12, cl13, cl23, seed):
    ...
    entry = {
        ...
        "seed": f"sweep_{seed}", "solver": "cpsat",
    }
...
def main():
    log("="*60)
    log(f"Seed sweep s={args.seed_start}-{args.seed_end} t={args.timeout}s {args.workers}w sym={bool(args.symbreak)}")
