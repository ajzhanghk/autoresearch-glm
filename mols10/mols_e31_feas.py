#!/usr/bin/env python3
"""Feasibility attack targeting E<=28 from E=31 warm-start."""
...
from mols_cpsat_worker import (
    count_clashes, N,
    TRIPLE_MISS_FILE, POOL_LOCK, RESULTS_DIR, FOUND_FILE, build_and_solve
)

LOG_FILE = REPO / "mols10/results/l3_e31_feas.log"
...
def save_to_pool(L1, L2, L3, cl12, cl13, cl23, tag):
    ...
    entry = {
        ...
        "seed": f"feas31_{tag}", "solver": "cpsat_feas",
    }
...
def main():
    log("="*60)
    log("Feasibility attack targeting E<=28 from E=31 warm-start")
    ...
    log(f"Starting from E={session_best}")

    configs = []
    for target in [28, 26, 24, 22, 20, 18, 15]:
        for rseed in range(200, 220):
            configs.append((target, 600, 8, True, rseed))
    for target in [28, 26, 24]:
        for rseed in range(220, 240):
            configs.append((target, 1200, 8, True, rseed))
    ...
    for target, timeout, workers, symbreak, rseed in configs:
        ...
        result = build_and_solve(L1, L2, N, hint_L3=hint,
            timeout_s=timeout, num_workers=workers,
        ...
        if L3_sol is not None:
            ...
            if is_optimal and E > 0:
                log(f"  PROVED: minimum E>={E} for this (L1,L2) pair")
        else:
